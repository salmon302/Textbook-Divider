...
class TestSchoenbergPage42(unittest.TestCase):
	...
	def test_text_extraction(self):
		...
		stages = {
			...
			"contrast": ImageEnhance.Contrast(pil_image).enhance(3.0),
			"sharpness": ImageEnhance.Sharpness(pil_image).enhance(3.0),
			"grayscale": ImageEnhance.Contrast(pil_image.convert('L')).enhance(2.5),
			"brightness": ImageEnhance.Brightness(pil_image).enhance(2.0),
			"bw": ImageEnhance.Contrast(pil_image.convert('L')).enhance(4.0)  # Higher contrast B&W
		}
		...
		for stage_name, stage_img in stages.items():
			try:
				...
				# Add deskewing and noise reduction
				if isinstance(stage_img, Image.Image):
					img_array = np.array(stage_img)
					if len(img_array.shape) == 3:
						img_array = cv2.cvtColor(img_array, cv2.COLOR_RGB2GRAY)
					# Denoise
					img_array = cv2.fastNlMeansDenoising(img_array)
					# Convert back to PIL
					stage_img = Image.fromarray(img_array)
